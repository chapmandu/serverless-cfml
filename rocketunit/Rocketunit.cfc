...
component {
  ...
  public boolean function runTestPackage(string testPackage) {
    ...
    directory action="list"
		...
		qDirectoryListing.filter((row) => {
			if (listContainsNoCase("Application.cfc,Test.cfc", row.name)) {
				return false;
			} else if (row.directory does not contain "/tests/") {
				return false;
			};
			return true;
		})
        ...
        if (
						listLast(componentMetadata.extends.name, ".") eq TEST_PREFIX
    ...
    metadata.functions.map(function(fn){
      ...
      try {
        ...
        if (cfcatch.errorcode eq FAIL_ERRCODE) {
          ...
          numTestFailures++;
          results.numFailures++;
					header statusCode = 417;
        } else {
          // general exception
          status = ERR_STATUS;
          message &= NEWLINE
                  & listLast(cfcatch.tagContext[1].template, "/")
                  & " line "
  ...
  public string function HTMLFormatTestResults(struct results, string id="results") {
	...
	/**
    JSONFormatTestResults
   **/
	public string function JSONFormatTestResults(struct results) {
    return SerializeJSON(arguments.results);
  }

	/**
    TestFormatTestResults
   **/
	public string function TextFormatTestResults(struct results) {
		queryPrinter = new QueryPrinter();

		cases = queryNew("package,tests,failures,errors");
		for (package in results.cases) {
			QueryAddRow(cases, {
				package: package.testCase,
				tests: package.numTests,
				failures: package.numFailures,
				errors: package.numErrors
			});
		}
		...
		failures = queryNew("package,test,message");
		results.tests.filter((r) => {
			return arguments.r.status != "Success";
		}).each((r) => {
			QueryAddRow(failures, {
				package: arguments.r.testCase,
				test: arguments.r.testCase,
				message: stripHTML(arguments.r.message)
			});
		});
		...
		response = [
			"SUMMARY",
			NewLine(),
			queryPrinter.ToString((cases)),
			NewLine(),
			NewLine()
		];
		if (!results.ok) {
			ArrayAppend(response, "FAILURES");
			ArrayAppend(response, NewLine());
			ArrayAppend(response, queryPrinter.ToString((failures)));
			ArrayAppend(response, NewLine());
			ArrayAppend(response, NewLine());
		}
		ArrayAppend(response, "OUTCOME:#results.ok ? 'SUCCESS' : 'FAILURE'#");
  ...
  public function onRequest() {
    ...
    if  (structKeyExists(url, JUNIT_URL_KEY)) {
      ...
      writeOutput(JUnitFormatTestResults(results));
		} else if  (structKeyExists(url, JSON_URL_KEY)) {
			content reset=true;
      writeOutput(JSONFormatTestResults(results));
		} else if  (structKeyExists(url, TXT_URL_KEY)) {
			content reset=true;
      writeOutput(TextFormatTestResults(results));
    } else {
      content reset=true;
